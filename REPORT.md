...
![GHCR Packages](ghcr-packages.jpg)
### 4. Выводы
В ходе выполнения практической работы я освоила команды Docker: работу с образами, контейнерами, томами и сетями. 

В процессе работы столкнулась с небольшой проблемой: была синтаксическая ошибка в JavaScript-коде index.html, из-за которой не отображались данные на странице. 

В итоге работы я поняла, что контейнеризация значительно упрощает развёртывание и воспроизводимость приложений, а автоматизация через CI/CD позволяет избежать ручных ошибок при сборке и публикации образов.
